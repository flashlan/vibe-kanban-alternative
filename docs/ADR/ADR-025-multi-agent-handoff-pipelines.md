...
- Persistent project memory via `mem0`, recalled and written agentically through the `memory_search` / `memory_save` MCP tools (see [ADR-028](/ADR/ADR-028-mem0-agentic-recall)).
...
- The next stage's agent is spawned with a fresh context window, receiving the target stage prompt and the path to `IMPLEMENTATION_PLAN.md`; it recalls `mem0` memory itself via `memory_search` when needed (see [ADR-028](/ADR/ADR-028-mem0-agentic-recall)).
