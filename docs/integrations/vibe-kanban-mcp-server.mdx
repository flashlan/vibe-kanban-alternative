...
a mem0 server (default `http://localhost:8000`, override with `MEM0_URL`). The
`user_id` is the repository slug, so every CLI working in the same repository —
opencode, Claude, and so on — reads and writes the same memory.
